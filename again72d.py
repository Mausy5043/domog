...
# daemon72d.py creates a graph of DHT22 readings.
...
# constants
PLOT_TITLE      = "Humidity & Temperature DHT22"
PLOT_Y1LABEL    = "Temperature [degC]"
PLOT_Y2LABEL    = "Humidity [%]"
...
def update_hour_query(consql, xdata, ydata, queryminutes):
  ...
  divider = 60
  sqlcmd = ('SELECT MIN(sample_time), AVG(temperature), AVG(humidity) '
            'FROM dht22 '
  ...
  for i, j, k in (data):
    ...
    if i > previ:
      ...
      ydata = np.append(ydata, [[float(j), float(k)]], axis=0)
...
def update_day_query(consql, xdata, ydata, queryhours):
  ...
  for i, j, k, l, m, n, o in (data):
    ...
    if i > previ:
      xdata = np.append(xdata, i)
      ydata = np.append(ydata, [[float(j), float(k), float(l), float(m), float(n), float(o)]], axis=0)
