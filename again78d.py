...
# daemon78d.py creates a windroos of wind speed and direction from Gilze-Rijen weatherstation data
...
# constants
PLOT_TITLE      = "Wind Speed & Direction (Gilze-Rijen)"
PLOT_Y1LABEL    = "Speed [km/h]"
PLOT_Y2LABEL    = "Direction [deg]"
...
def update_hour_query(consql, xdata, ydata, queryminutes):
  ...
  divider = 60
  sqlcmd = ('SELECT MIN(sample_time), AVG(speed), AVG(direction) '
            'FROM wind '
  ...
  for i, j, k in (data):
    ...
    if i > previ:
      ...
      ydata = np.append(ydata, [[kmh(float(j)), float(k)]], axis=0)
...
def update_day_query(consql, xdata, ydata, queryhours):
  ...
  divider = 1800
  sqlcmd = ('SELECT MIN(sample_time), MIN(speed), AVG(speed), MAX(speed), '
  ...
  for i, j, k, l, m, n, o in (data):
    ...
    if i > previ:
      xdata = np.append(xdata, i)
      ydata = np.append(ydata, [[kmh(float(j)), kmh(float(k)), kmh(float(l)), float(m), float(n), float(o)]], axis=0)
...
@timeme
def update_hour_graph(ymin, ymax, y2min, y2max):
...
def do_main(flock, nu, consql):
  ...
  if (currenthour == 1) and (currentminute == 11) or nu:
    ...
    if nu:
      ...
      yearly_data_y = np.array([[0, 0, 0, 0, 0, 0]])  # initialise array with dummy data
  ...
  if (currenthour % 4) == 0 and (currentminute == 1) or nu:
    ...
    if nu:
      ...
      weekly_data_y = np.array([[0, 0, 0, 0, 0, 0]])  # initialise array with dummy data
  ...
  if (currentminute % 30) == 0 or nu:
    ...
    if nu:
      ...
      daily_data_y = np.array([[0, 0, 0, 0, 0, 0]])  # initialise array with dummy data
  ...
  if (currenthour == 1) and (currentminute == 11) or nu:
    update_year_graph(minimum_y, maximum_y, minimum_y2, maximum_y2)

  # WEEK data
  # graph of the last week is updated every 4 hours
  if (currenthour % 4) == 0 and (currentminute == 1) or nu:
    update_week_graph(minimum_y, maximum_y, minimum_y2, maximum_y2)

  # DAY data
  # graph of the last day is updated every 30 minutes
  if (currentminute % 30) == 0 or nu:
    update_day_graph(minimum_y, maximum_y, minimum_y2, maximum_y2)
  ...
  if (currentminute % 3) == 0 or nu:
    ...
    plt.savefig('/tmp/domog/site/img/day79.png', format='png')
